...
// MARK: - Country Ad Filter Types
enum CountryAdMetricFilter: String, CaseIterable {
...
// MARK: - Country Ad Data Model
struct CountryAdData: Identifiable, Codable, Equatable {
    let id = UUID()
    ...
    // Helper method to create CountryAdData from AdMob API response
    static func fromAdMobResponse(_ row: AdMobReportRow) -> CountryAdData? {
        guard let dimensionValues = row.dimensionValues,
              let metricValues = row.metricValues else { return nil }
        
        // Extract country information from dimensions
        let countryDimension = dimensionValues["COUNTRY"]
        let countryName = countryDimension?.displayLabel ?? countryDimension?.value ?? "Unknown Country"
    ...
    func getValue(for filter: CountryAdMetricFilter, isDemoMode: Bool = false) -> String {
        switch filter {
        case .earnings:
