...
class CountryViewModel: ObservableObject {
    ...
    func fetchCountryData() async {
        ...
        let dateRange = selectedFilter.dateRange
        let result = await fetchCountriesData(
            accountID: accountID,
    ...
    private func fetchCountriesData(accountID: String, accessToken: String, startDate: Date, endDate: Date) async -> Result<[CountryData], AdSenseError> {
        ...
        let urlString = "https://adsense.googleapis.com/v2/\(accountID)/reports:generate?\(metricsQuery)&dimensions=COUNTRY_CODE&startDate.year=\(start.prefix(4))&startDate.month=\(start.dropFirst(5).prefix(2))&startDate.day=\(start.suffix(2))&endDate.year=\(end.prefix(4))&endDate.month=\(end.dropFirst(5).prefix(2))&endDate.day=\(end.suffix(2))"
        ...
        do {
            ...
            case 200:
                ...
                let countries = rows.compactMap { row -> CountryData? in
                    guard let cells = row["cells"] as? [[String: Any]] else { return nil }
                    
                    // The first cell contains the country code
                    let countryCode = cells.first?["value"] as? String ?? "Unknown"
                    ...
                    // The order should be: country_code, earnings, requests, page_views, impressions, clicks, ctr, rpm
                    if cells.count >= 2 {
                    ...
                    return CountryData(
                        ...
                        countryName: "", // Will be computed from country code
                        earnings: earnings,
                        requests: requests,
                        pageViews: pageViews,
                ...
                return .success(sortedCountries)
